...
export default function MovieDetailAdminPage({
  ...
  return (
    <div>
      <h1 className={title()}>Quản lý phim {detailData?.movieDetail?.name}</h1>
      <Breadcrumbs
        className="my-4"
        itemClasses={{
          item: "px-2",
      ...
      >
        ...
        <BreadcrumbItem href={`/admin/movie/${detailData?.movieDetail?.movieid}`}>{detailData?.movieDetail?.name}</BreadcrumbItem>
      </Breadcrumbs>
      <div className="flex lg:flex-row flex-col-reverse">
        <div className="lg:w-4/5 lg:mr-8">
          <div className="flex flex-row">
            {detailData?.list?.length > 0 && <EpisodeCard cardData={detailData?.list?.[0]} />}
            <AddNewEpisodeAdminForm />
          </div>
        </div>
        <div className="lg:w-1/5 mb-4">
          <AdminForm adminFormCofig={adminFormCofig} rerenderData={detailData?.movieDetail} />
        </div>
      </div>
    </div >
  );
